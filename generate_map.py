...
Automatically fetches logbook data from QRZ.com API and generates interactive map
"""
...
# Configuration
QRZ_USERNAME = os.environ.get('QRZ_USERNAME', 'TA1ZMP')
...
def get_qrz_session():
    ...
    if session is not None:
        ...
        print(f"QRZ Auth Error: {error.text if error is not None else 'Unknown'}")
        return None

def fetch_logbook_data(session_key):
    """Fetch logbook data from QRZ API"""
...
<head>
    ...
    <title>TA1ZMP QSO Interactive Map</title>
...
<body>
    <div class="header">
        <h1>📡 TA1ZMP QSO Interactive Map</h1>
    ...
    <script>
        ...
        function updateMap() {{
            ...
            traces.push({{
                ...
                text: '<b>TA1ZMP</b><br>Beşiktaş, İstanbul',
                name: 'TA1ZMP (Home)',
                hovertemplate: '%{{text}}<extra></extra>'
            }});
...
def main():
    ...
    if not session_key:
        print("❌ Failed to authenticate with QRZ API")
        sys.exit(1)

    print("✅ Authentication successful")
    print("📥 Fetching logbook data...")

    adif_data = fetch_logbook_data(session_key)

    if not adif_data:
        print("❌ Failed to fetch logbook data")
        sys.exit(1)

    print("📊 Parsing ADIF data...")
    df = parse_adif(adif_data)

    if df.empty:
        print("⚠️ No QSO records found")
        sys.exit(0)
